...
import { getCollection } from '../db.js'; // Sobe um nível para encontrar o db.js
...
router.post('/update-password', verificarToken, async (req, res) => {
  try {
    ...
    res.json({ message: 'Password updated com sucesso!' });
  } catch (error) {
...
// 2. ROTA POST - Inserção de Desejo (REFEITA COM COOLDOWN FLEXÍVEL)
router.post('/desejos', verificarToken, async (req, res) => {
  try {
    const { nome, preco, categoria, cooldownValue, cooldownUnit, diasCooldown } = req.body;
    const dataLiberacao = new Date();

    // Lógica para lidar com minutos, horas, dias ou meses dinamicamente
    if (cooldownValue && cooldownUnit) {
      const valor = parseInt(cooldownValue || 0);
      switch (cooldownUnit) {
        case 'minutos':
          dataLiberacao.setMinutes(dataLiberacao.getMinutes() + valor);
          break;
        case 'horas':
          dataLiberacao.setHours(dataLiberacao.getHours() + valor);
          break;
        case 'dias':
          dataLiberacao.setDate(dataLiberacao.getDate() + valor);
          break;
        case 'meses':
          dataLiberacao.setMonth(dataLiberacao.getMonth() + valor);
          break;
        default:
          dataLiberacao.setDate(dataLiberacao.getDate() + valor);
      }
    } else {
      // Fallback para manter compatibilidade com o formato antigo (Apenas dias)
